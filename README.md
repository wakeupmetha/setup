...
- **Сеть.** Интерфейс дефолтного маршрута, скорость линка из `/sys/class/net/*/speed` и объём трафика с момента загрузки из `/proc/net/dev`. Всё читается мгновенно, без сэмплирования. На virtio-сетевухах скорость линка часто не сообщается — тогда выводится только трафик. Это не спидтест: замер полосы занял бы секунды и сжёг бы трафик, а для разового замера есть `speedtest`.
