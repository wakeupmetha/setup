...
| `ani` | anifetch с гифкой из `assets/` — **команда в `/usr/local/bin`, а не функция**, работает сразу после установки без `source` |
...
```
   ...
   10.0.0.5
   5.181.x.x
   Frankfurt am Main, DE - AS58087 Florian Kolb
   eth0 1000Mb/s  down 1.2 TiB  up 861.6 GiB
   943 down / 512 up Mbps  4ms  (29.07 03:11, 486 MiB used)
...
- **Время.** Слева время сервера с его таймзоной. Если геолокация показывает другую зону, справа через `|` добавляется время в городе, где стоит сервер — у большинства VPS системная зона UTC, а сервер физически во Франкфурте. Одинаковые зоны — вторая половина не рисуется. Таймзона города берётся из того же кэша, что город и провайдер.
- **Город.** В строке `geo`: город, страна, провайдер.
- **Сеть.** Интерфейс дефолтного маршрута, скорость линка из `/sys/class/net/*/speed` и объём трафика с момента загрузки из `/proc/net/dev`. Всё читается мгновенно. На virtio-сетевухах скорость линка часто не сообщается — тогда выводится только трафик.
- **Спидтест.** Последний замер `cloudflare-speed-cli --json`: down/up в Мбит/с, idle-латентность, когда мерили и **сколько трафика съел сам замер**. Гоняется тем же суточным таймером, панель только читает результат.
- **Shell.** Не модулем fastfetch, а отдельной командой: fastfetch запускается из anifetch и в поле shell честно пишет `anifetch`. Строка берёт `$SHELL` и спрашивает у него версию.

**Спидтест жрёт реальный трафик.** Настройки в `/etc/vps-set/fetch.conf` — файл создаётся один раз и повторными прогонами не перезаписывается:
...
Тест насыщает канал на указанное время, то есть цена ≈ скорость линка × длительность. На гигабитной ноде это ~500 МБ за прогон, ~15 ГБ в месяц. Дефолт я поставил втрое короче штатных 10s/10s именно поэтому. Выключить — `SPEEDTEST=0`, замерить руками в любой момент — `speedtest`.
...
Настройки анимации лежат в трёх строках вверху `/etc/profile.d/99-vps-set.sh` — одинаковые на всех нодах, потому что их пишет скрипт:

```bash
ANIFETCH_FILE="playboi-carti.gif"
ANIFETCH_CHAFA="--symbols block --fg-only -c 256"
ANIFETCH_LOOP="-1"
```

Правь прямо там — повторный прогон `meth-setup shell` твои значения не затрёт, а старым файлам без этих ключей проставит дефолты. Разово, без правки файла:

```bash
ANI_CHAFA="--symbols braille -c full" ANI_LOOP=3 ani
```
...
**Почему на разных серверах выглядит по-разному.** Не из-за Sixel/Kitty — anifetch жёстко пинит `--format symbols` в двух местах кода (это фикс их issue #1), так что графические протоколы не задействуются вообще и вывод всегда обычный текст с SGR-цветом. Различается **глубина цвета**: `chafa -c` документирован как «Defaults to best guess» и определяется по `TERM`/`COLORTERM`, то есть зависит от клиента, а не от сервера.

Поэтому в дефолт зашит `-c 256` — автоопределение отключено, и одна и та же гифка выглядит одинаково с любого клиента и на любой ноде. Режимы: `none, 2, 8, 16/8, 16, 240, 256, full`. Хочешь максимум цвета ценой одинаковости — ставь `-c full` в `ANIFETCH_CHAFA`.

**Если терминал виснет при скролле** (Termius этим страдает — свой рендер, не тянет одновременно поток перерисовки и прокрутку истории), не давай анимации крутиться бесконечно:

```bash
ANI_LOOP=2 ani
```

По умолчанию `-1`, то есть до нажатия клавиши. С конечным счётчиком анимация досматривается и терминал возвращается в обычный режим.
